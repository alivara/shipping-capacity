...
class TestETLPipelineTransform:
    ...
    @pytest.mark.asyncio
    async def test_transform_complex_deduplication_scenario(self):
        """
        Test complex scenario with both exact duplicates and journey duplicates.

        This tests the complete deduplication workflow:
        1. Remove exact duplicates
        2. Keep latest for duplicate journeys
        """
        df = pd.DataFrame(
            {
                "ORIGIN": [
                    "china_main",
                    "china_main",  # Exact duplicates
                    "china_main",
                    "china_main",  # Same journey, different times
                    "china_main",  # Unique journey
                ],
                "DESTINATION": ["north_europe_main"] * 5,
                "ORIGIN_PORT_CODE": ["CNSHA", "CNSHA", "CNSHA", "CNYTN", "CNNGB"],
                "DESTINATION_PORT_CODE": ["NLRTM", "NLRTM", "NLRTM", "NLRTM", "DEHAM"],
                "SERVICE_VERSION_AND_ROUNDTRIP_IDENTFIERS": [
                    "SERVICE_1",
                    "SERVICE_1",  # Exact duplicates
                    "SERVICE_2",
                    "SERVICE_2",  # Same journey
                    "SERVICE_3",  # Unique
                ],
                "ORIGIN_SERVICE_VERSION_AND_MASTER": [
                    "MASTER_1",
                ...
                "ORIGIN_AT_UTC": [
                    "2024-01-01",
                    "2024-01-01",  # Exact duplicates
                    "2024-01-01",
                    "2024-01-03",  # SERVICE_2: keep later
                    "2024-01-02",
                ],
                "OFFERED_CAPACITY_TEU": [15000, 15000, 16000, 17000, 18000],
            }
        )
        ...
        # Should have 3 rows: SERVICE_1 (deduped), SERVICE_2 (latest only), SERVICE_3
        assert len(df_transformed) == 3

        # Verify SERVICE_2 kept the latest (2024-01-03)
        service_2_rows = df_transformed[
            df_transformed["service_version_and_roundtrip_identfiers"] == "SERVICE_2"
        ]
        assert len(service_2_rows) == 1
        assert service_2_rows.iloc[0]["offered_capacity_teu"] == 17000
